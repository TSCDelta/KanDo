...
import { X, Plus, Check, Lock, LockOpen, ArrowLeft } from "lucide-react";
import { createNewBoard } from "@/lib/helper";
import { bannerPresetCategories, BannerCategoryKey } from "@/lib/bannerPresets";
...
export default function BoardCreationModal({
  ...
  const [moreBanners, setMoreBanners] = useState<boolean>(true);
  const [selectedCategory, setSelectedCategory] =
    useState<BannerCategoryKey>("nature");
      ...
      <div className="relative w-full max-w-lg rounded-xl bg-background p-6 shadow-xl border border-border animate-fadeIn">
        {moreBanners ? (
          <>
            ...
            <p className="text-sm mb-2 font-medium">Privacy</p>
            <div className="flex w-full rounded-lg border border-border overflow-hidden mb-6">
              <button
                type="button"
                onClick={() => setPrivacy("private")}
                className={`flex-1 py-2 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  privacy === "private"
                    ? "bg-background-selected text-white"
                    : "bg-background-alt text-gray-400 hover:text-gray-200"
                }`}
              >
            ...
            <div className="flex justify-between">
              <button
                className="px-4 py-2 rounded-md bg-background-alt hover:bg-border-hover text-sm font-medium"
          ...
          <>
            ...
            <p className="text-sm mb-2 font-medium">Browse Backgrounds</p>
            <div className="flex gap-2 mb-3 flex-wrap">
              {bannerPresetCategories.map((cat) => (
                <button
                  key={cat.key}
                  type="button"
                  ...
                  className={`px-3 py-1 rounded-md text-xs border ${
                    selectedCategory === cat.key
                      ? "bg-background-selected text-white border-border"
                      : "bg-background-alt text-gray-300 border-border hover:text-gray-100"
                  }`}
                >
                  {cat.label}
                </button>
